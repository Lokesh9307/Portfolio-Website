...
const ContactLink = () => {
  ...
  return (
    <div className=' lg:mb-5 mb-0 bg-transparent md:w-[10rem] w-full'>
      <h1 className='lg:text-end text-center text-white/60 text-lg my-2'>Social Contacts</h1>
      <section className='flex lg:gap-5 font-bold gap-2 bg-transparent lg:flex-col items-center justify-end'>
        {ContactLinks.map((items, id) => (
          <>
            <Link key={id} target='blank' className='tooltip flex items-center gap-2 lg:flex-row flex-col bg-transparent lg:justify-between justify-around group' href={items.link}>
              <span key={id} className='hover:bg-[#343a40c0] p-2 rounded-lg bg-transparent'>{items.icon}</span></Link>
          </>
        ))}
      </section>
    </div>
